...
interface CalculatorCommandExecutor {
    static void execute(FileInputStream inputStream, HashMap<String, String> commands) {
        ...
        try (BufferedReader fileReader = new BufferedReader(new InputStreamReader(inputStream))) {
            ...
            while (line != null) {
                ...
                if (CalculatorCommandSyntaxChecker.isCommandFound(splitedLine, commands)) {
                    var currentOperation = CalculatorCommandFabric.createNewMathOperation(commands.get(splitedLine[0].toUpperCase()));
